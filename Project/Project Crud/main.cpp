...
void deleteRecord(fstream &data){
	...
	dataSementara.open("temp.text", ios::trunc|ios::out|ios::in);
	...
	data.close();
	data.open("data.text",ios::trunc|ios::out);
	data.close();
	data.open("data.text",ios::out|ios::in);
...
void checkDatabase(fstream &data){
	...
	if (data.is_open()){
		...
		data.close();
		data.open("data.text", ios::trunc | ios::out | ios::in);
	}
}
